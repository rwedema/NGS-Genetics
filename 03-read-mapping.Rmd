...
Because our sequence reads are from captured exomes, you have to zoom in quite a bit to see any of the mappings. Too help you find where to zoom in, we can add an extra layer to the genome browser (called a track). The required BED file (*Browser Extensible Data*) is available on the network in the same location where the read files are stored: `/commons/Themas/Minor01/Genomics/` where you can navigate to either the `CardioPanel` or `MedulloblastomaPanel` folder. In this folder, there is a `.bed` file with all positions of all sequenced genes/exons of the gene panel. Upload this file (use the `V3_ucsc` file for Medulloblastoma) into Galaxy and please have a look at the contents in you History. The file consists of 4 columns, which describe the chromosome number, exon start location, exon end location and the gene name. Download this file (*Save File*) to your computer.
