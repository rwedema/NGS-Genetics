...
2. Create a `ScanBamParam` object
    ...
    * Give this `GRanges` object to the `ScanBamParam` function (using the `which` parameter) and store this in a variable

3. Create a `PileupParam` object
  * We use this to make sure the output data of the `pileup` function (next step) is as we want
  * Set the `distinguish_strands` option to `FALSE`; this will give us the total count per position, independent of the direction (strand) the read is mapped
  * Set the `distinguish_nucleotides` option to `FALSE`; we are not interested (now) in reporting on variants
  
4. Get the actual mapping data for our region of interest
    * Call the `pileup` function providing - in order - the objects created at points 1-3. Note that you need to tell `pileup` what the parameter is, so use `pileup(file = ..., scanBamParam = ..., PileupParam = ...)`
...
3. Get the pileup data for each gene in the for-loop, by creating `ScanBamParam` and `PileupParam` variables and executing the `pileup` function
...
Calculating the actual coverage for each position is fairly easy by just using the `count` column from the `pileup` output. Your task now is to calculate this per-position coverage for all genes and to calculate and report on - for each gene:
...
Given these tasks and your code from previous assignments, you should be able to do this. Store the calculated values combined with the gene name in a new data-frame and show it completely in your lab-journal, ordered by the percentage of low-coverage positions, descending.

The following code demonstrates how to create an empty data-frame and add rows to the data, that can be used in your for-loop. Note that this isn't done very often, use and forget. The first line of code (the `options` function) is needed to stop some annoying R behaviour.
