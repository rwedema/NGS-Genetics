...
3. Perform the following steps on the column containing the frequency value:
    ...
    * converts the data type of this column to `numeric` (note that you cannot convert a column in a matrix to a different data type, first convert to a data frame instead)
...
* Visualize the amount of variants per gene. A simple solution for getting the numbers is to use the `table` function on the `subjectHits` column (output of `findOverlaps` where `subject` are the *genes*). Convert it to a data frame and then use the gene indices to get the actual gene names. Now you should have a combination of gene and number of variants. Create a barplot with this data as in the previous chapter.
