...
Run the `Call variants with LoFreq` tool using its default settings. Select the output BAM file from the Mark Duplicate tool and use the available `hg38` reference genome. Feel free to explore the other options but leave the `Call variants across` settings on `Whole reference`.

## VCF File Filtering {#variant-filtering}

The result of the LoFreq tool is a single new file in your history in the [**vcf**](http://www.internationalgenome.org/wiki/Analysis/vcf4.0/) file format where each line describes a single variant. In Galaxy you can then directly see how many variants you have; often well over 1000 in total. Note the comment-lines at the top of the file.

When looking at this file in more detail it is fairly easy to see variants in places we are not interested in (you can also add the VCF file as a track in IGV to spot detected variants). As you've seen in IGV, many regions have been sequenced outside of our genes of interest, or even very far from a gene at all. LoFreq also checked those regions for variants and this assignment asks to filter the list of variants only keeping those within exon boundaries of our genes of interest. Then, another filtering step is filtering on actual variants. We do this by looking at the [allele frequency](https://en.wikipedia.org/wiki/Allele_frequency) value included in the VCF file. In our case, this value describes the percentage of reads having the **variant** base.
...
1. Read in the data into a data-frame using the `read.delim` function
    * Make sure the data has a proper header (skip all header lines starting with a `#`)
...
The output of this function is a list in which each item is a vector with the separate items resulting from the split (inspect this object in RStudio, for instance, by entering the variable name in the console (and pressing Enter)). To get the frequency value from this list, here is some R magic that converts the result from `strsplit` into a matrix (found on [Stackoverflow](https://stackoverflow.com/questions/20428742/select-first-element-of-nested-list)) so that we can access the value in a column:
...
3. Perform the following steps on the column containing the frequency value:
    ...
    * convert the data type of this column to `numeric` (note that you cannot convert a column in a matrix to a different data type, first convert to a data frame instead)
    * save only this column into a new variable
...
```{r, echo=TRUE}
# Example: generate some random values for 'allele frequency'
...
5. Get all variants that fall within an exon
    ...
    * Convert the output to a data-frame using the `as.data.frame` function
...
6. Filter the **remaining** variants based on their frequency, using a minimum of **30%**
    ...
    * Do this by sub-setting the `queryHits` column (this is a bit of a puzzle)
    * Manually sample a few rows to see if they do fall within an exon and have a frequency > 30%

7. Now that you have all the row numbers of variants that we want to keep, you need to think of a way to re-construct the VCF file and save it to disk. This means that we need to add all the header lines and all the filtered variants
    * Hint: read in the VCF file again for sub-setting using the `readLines` function; this gives a *vector* where each element is a line in the file. 
      - Example: if we have 100 header lines and our `queryHits` state that a variant in position 10 has overlap with an exon, we want to keep line $100 + 10 =110$. 
    * Reconstruct the VCF file:
      - Combine the header (i.e. `header <- raw_vcf[1:100]`) and the filtered variants (i.e. `final_filtered_vcf <- c(header, variants)`)
      - Write this to a new file using the `writeLines` function (provide a file name with its `con` argument)
...
* Visualize the amount of variants per gene. A simple solution for getting the numbers is to use the `table` function on the `subjectHits` column (output of `findOverlaps` where `subject` are the *genes*). Convert it to a data frame and then use the gene indices to get the actual gene names. Now you should have a combination of gene and number of variants. Create a bar plot with this data.

It is also possible to add the VCF file to IGV (together with the mapping- and BED-files) and visualize the variants in the context of the genes. This is however not a requirement for this assignment. When adding both the original and filtered VCF files, it's easy to spot which variants were filtered out and which were retained after filtering.
